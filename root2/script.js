...
document.addEventListener('keydown', (e) => {
  switch (e.keyCode) {
    case 69: // "e"
      ...
      if (eraseMode) document.getElementById('mode').innerHTML = 'Erase mode';
      else document.getElementById('mode').innerHTML = 'Create mode';
      break;
    case 90: // "z"
      if (!eraseMode) {
        const val = targets.pop();
        if (val !== undefined) resetTargets.push(val); // delete last target and save it for "y"
    ...
    case 89: // "y"
      if (!eraseMode) {
        ...
        if (val !== undefined) targets.push(val); // delete last target and save it for "y"
